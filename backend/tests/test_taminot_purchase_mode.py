"""Ta'minot: olib kelish turi — qarzga yoki naqd.
  ...
  - "debt" (sukut) — qarz qoldig'i summa qadar oshadi
  - "cash"         — shu zahoti to'lov yoziladi, qarz qoldig'i o'zgarmaydi

Ikkala holatda ham ombor qoldig'i bir xil oshadi.
...
async def _product(c, **kw):
    payload = {"scope": "ichki", "name": "Profil 40x40", "unit": "metr",
...
async def _get(c, pid):
    rows = (await c.get(f"{API}/products", params={"scope": "ichki"})).json()
    return next(p for p in rows if p["id"] == pid)


async def test_debt_purchase_increases_balance(client, db_engine):
    """Qarzga olib kelish — sukut bo'yicha: qarz ham, qoldiq ham oshadi."""
    ...
    assert fresh["total_paid"] == 0
    assert fresh["balance"] == 10000
...
async def test_cash_purchase_leaves_no_debt(client, db_engine):
    ...
    assert fresh["total_purchased"] == 10000
    assert fresh["total_paid"] == 10000   # avtomatik to'lov
    assert fresh["balance"] == 0          # qarz qolmaydi
...
async def test_mixed_purchases_sum_correctly(client, db_engine):
    ...
    assert fresh["total_paid"] == 5000     # faqat naqd qismi
    assert fresh["balance"] == 7000        # qarzga olingan qismi
