...
`GET /bot-menu/agent` orqali oladi. Mijoz bo'limni uch yo'l bilan tanlaydi:
...
Prod'da agent bir nechta worker jarayonida ishlaydi, ERP esa «yangila»
xabarini ulardan faqat BIRIGA yuboradi. Shuning uchun olingan menyu umumiy
holatga (Redis) versiyasi bilan yoziladi va har bir worker webhook kelganda
versiyani solishtirib, kerak bo'lsa o'zini yangilaydi.
...
# --------------------------------------------------------------------------- #
# ERP'dan olish, workerlar orasida ulashish, Telegram buyruqlarini moslash
# --------------------------------------------------------------------------- #
def _erp_base() -> str:
    # ".../api/v1/leads/ingest" -> ".../api/v1"
    return settings.ERP_INGEST_URL.rsplit("/leads/ingest", 1)[0]
...
async def refresh() -> Menu:
    """Menyuni ERP'dan qayta oladi. Xato bo'lsa eski menyu qoladi.

    Buyruqlar faqat menyu MUVAFFAQIYATLI olinganda moslanadi — aks holda ERP
    vaqtincha javob bermay qolsa botdagi buyruqlar o'chib ketardi.
    """
    if not settings.AGENT_INGEST_KEY:
        return _state.menu
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get(f"{_erp_base()}/bot-menu/agent", headers=_headers())
        data = resp.json() if resp.status_code == 200 else None
    except (httpx.HTTPError, ValueError) as exc:
        logger.warning("Bot menyusini olishda xato (eski menyu qoladi): {}", exc)
        return _state.menu
    if data is None:
        logger.warning("Bot menyusini olib bo'lmadi ({}): {}", resp.status_code, resp.text[:150])
        return _state.menu
    menu = _parse(data)
    if menu is None:
        return _state.menu
...
async def sync_commands() -> None:
    """«Menu» tugmasidagi buyruqlarni ERP'dagi bo'limlarga moslaydi (o'zgarsa)."""
...
async def _download(image: MenuImage) -> Optional[tuple[bytes, str]]:
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            resp = await client.get(f"{_erp_base()}/bot-menu/agent/images/{image.id}",
                                    headers=_headers())
        if resp.status_code == 200 and resp.content:
            return resp.content, image.content_type
        logger.warning("Menyu rasmi olinmadi ({}): {}", resp.status_code, image.id)
    except httpx.HTTPError as exc:
        logger.warning("Menyu rasmini olishda xato: {}", exc)
    return None


async def _photos(images: tuple[MenuImage, ...], *, use_cache: bool) -> tuple[list[PhotoInput], bool]:
    """Rasmlar: keshdagi `file_id` yoki ERP'dan baytlar. (ro'yxat, keshdan olinganmi)."""
