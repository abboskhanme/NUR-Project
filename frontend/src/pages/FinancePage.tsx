...
export default function FinancePage() {
  ...
  return (
    <div className="space-y-4">
            ...
            {txItems.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {txItems.map((t) => (
                      <tr key={t.id} className="border-b border-black/5 hover:bg-black/5 group">
                        <td className="py-2 pr-3 whitespace-nowrap">{formatDate(t.date)}</td>
                        <td className="py-2 pr-3"><TypeBadge type={t.type} /></td>
                        <td className="py-2 pr-[3.75rem] text-ink-soft whitespace-nowrap">{t.category_name || '—'}</td>
                        <td className={`py-2 pr-[3.75rem] font-semibold whitespace-nowrap ${
                          t.type === 'income' ? 'text-success' : 'text-danger'}`}>
                          {t.type === 'income' ? '+' : '−'}{fmtMoney(t.amount, t.currency)}
                        </td>
                        <td className="py-2 pl-2 w-full text-ink-soft">{t.note || '—'}</td>
                        <td className="py-2">
                          <button onClick={() => setDelTx(t)}
      ...
      <ConfirmModal open={!!delTx} title="Tranzaksiyani o'chirish"
        message="Bu tranzaksiya o'chiriladi va balans qaytariladi. Davom etilsinmi?"
