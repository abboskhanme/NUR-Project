...
const RECEIPT_LABEL: Record<string, string> = {
  open: 'Qarz', partial: 'Qisman', paid: 'To\'langan',
};
const RECEIPT_STYLE: Record<string, string> = {
  open: 'bg-danger/10 text-danger', partial: 'bg-warning/10 text-warning',
...
export default function SupplyPage() {
  ...
  // Taminotchi rejimi: joriy foydalanuvchi biror vendorga bog'langanmi?
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Kam qolgan zaxira bannerini ko'rsatmaymiz — KPI va belgilar yetarli */}
      ...
      <div className="flex gap-1 border-b border-black/5">
        {([['items', 'Mahsulotlar'], ['receipts', 'Kirimlar / Qarzlar']] as const).map(([k, lbl]) => (
                    ...
                    <tr key={r.id} className="border-b border-black/5 hover:bg-black/5">
                      ...
                      <td className="py-2 pr-3">
                        <span className={cn('badge', RECEIPT_STYLE[r.status] || 'bg-gray-100 text-gray-700')}>
                          {RECEIPT_LABEL[r.status] || r.status}
                        </span>
                      </td>
                    </tr>
                  ))}
