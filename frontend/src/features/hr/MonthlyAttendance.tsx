...
interface DayRow {
  ...
  checkIn: string;
  checkOut: string;
  locked: boolean; // hire_date'dan oldin
}
...
export default function MonthlyAttendance({
  ...
  function fillAll() {
    // Yakshanbalar (weekday === 0) to'ldirilmaydi
    setRows((prev) =>
      prev.map((r) =>
    ...
    <Card>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-base w-36 text-center">{MONTHS[month - 1]} {year}</h3>
          <button onClick={() => onShiftMonth(1)} className="p-1.5 rounded hover:bg-black/5">
      ...
      <div className="flex items-end gap-x-6 gap-y-3 flex-wrap mb-4 p-4 bg-black/[0.02] rounded-button">
        ...
        <div className="flex items-center gap-2">
          <button onClick={fillAll} className="btn-ghost"><Wand2 size={15} /> Hammasini to'ldirish</button>
                ...
                return (
                  ...
                  >
                    <td className="py-1.5 pr-6 whitespace-nowrap min-w-[160px]">
                      <span className="font-medium">{r.day} {MONTHS[month - 1]}</span>
                      <span className="text-ink-soft text-xs ml-2">{WD[r.weekday]}</span>
                    </td>
      ...
      <ConfirmModal
        ...
        message={`${MONTHS[month - 1]} ${year} oyidagi barcha kelish-ketish vaqtlari tozalanadi. Davom etasizmi? (O'zgarish "Saqlash" bosilgandan keyin kuchga kiradi)`}
